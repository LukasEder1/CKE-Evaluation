...
def parse_html_to_string(article_id, conn):
    
    data = pd.read_sql(f"SELECT * FROM split_sentences where entry_id = {article_id}", conn)
    ...
    for version in versions:
        ...
        documents.append(BeautifulSoup(text).get_text())
